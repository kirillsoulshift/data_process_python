...
изображений в новую директорию '_ldf',"""
...
def get_name(path):
    ...
    try:
        ...
        raise FileNotFoundError(f'В пути {path} не обнаружен текстовый файл <name.txt> с названием страницы') from None


def make_catnum(s):
    """Производит запись пустой строки в файл
    если значение партномера в имени папки 'NA'"""
    if s == 'NA':
        return ''
    else:
        return s
...
def entry(s):
    """Используется для записи текстовой строки
    в файл ldf"""
    if s.count('\n'):
        s = s.replace('\n', ' ')
...
def get_pics(path):
    """Перемещает изображения из папки в
    директории каталога в папку с ldf файлами"""
    pics = []
...
def entity_define(path):
    ...
    if ent_count['files'] > 1:
        raise ValueError(f'Более одного файла .xlsx в папке {path}') from None
    return ent_count


def get_ref(folder_path, cat_num: str, target_val: str):
    """
    возвращает номер ссылки по совпадению каталожного номера,
    если его нет, по зyачению строки в столбце translate,
    если его нет, по зyачению строки в столбце description,

    :param folder_path: pathlike object, папка в которой производится поиск вложенных папок
    :param cat_num: партномер строки
    :param target_val: str наименование строки на русском
    :return: str порядковый номер папки, который помещается в графу ссылка в ldf
    """
    if cat_num != cat_num:
        raise Exception(f'NAN в контенте {folder_path}') from None
    ...
    if cat_num != '':
        ...
        if len(reference_list) == 0:
            ...
            raise ValueError(f'Дублирование каталожных номеров в папке {folder_path}') from None
    else:
        if target_val != '':
            reference_list = []
            for item in content:
                if re.search(target_val[:10], item):
                    reference_list.append(item.split(' ')[0])
            if len(reference_list) == 0:
                return ''
            elif len(reference_list) == 1:
                return reference_list[0]
            else:
                print(f'Дублирование наименований на русском языке (без part_№) в xlsx в папке {folder_path},'
                      f'требуется проверка расстановки ссылок в ldf {os.path.basename(folder_path).split(" ")[0]}')
                return reference_list[0]
        else:
            return ''
...
def write_ldf(path, dest_path, content_dict, need_translate: bool, need_comment: bool):
    """
    ...
    :param need_translate: востребованность столбца перевода
    :param need_comment: востребованность столбца комментария
    :return: None
    """
    ...
    if content_dict['files'] != 0:  # если ldf файл делается из .xlsx
        ...
        if len(xlsx_list) != 1:
            raise ValueError(f'Неверное количество файлов .xlsx в папаке {path}') from None
    ...
    with open(ldf_path, 'wb') as ldf:
        ...
        if content_dict['files'] != 0:  # если ldf файл делается из .xlsx
            ...
            for n, row in enumerate(raw_file.values.tolist()):
                ...
                ldf.write(b',,,,')
                if content_dict['folders'] != 0:  # если директория содержит папки в ldf прописывается ссылка
                    ldf.write(get_ref(path, row[1].strip(' "'), row[5].strip(' "')).encode('cp1251'))
                    reference_count += 1
                ldf.write(b',')
...
def folder_surf(path, dest_path, translate: bool, comment: bool):
    """Рекурсивная обработка папок в директории каталога,
    запуск функции write_ldf в каждой из них"""
    content = os.listdir(path)
    ent_count = entity_define(path)
    write_ldf(path, dest_path, ent_count, translate, comment)
    for item in content:
        if os.path.isdir(os.path.join(path, item)):
            folder_surf(os.path.join(path, item), dest_path, translate, comment)
